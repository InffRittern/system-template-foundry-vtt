import { type GluegunToolbox, build, filesystem, print } from "gluegun";
...
function symLinkToFixtures() {
	...
	filesystem
		...
		.catch((err) => {
			print.error(
				"Failed to create a symlink to the fixtures directory!"
			);
			throw err;
		});
}
...
const cli = build("setup")
	...
	.defaultCommand({
		run: async (tools: GluegunToolbox) => {
			...
			print.highlight("Welcome to System Development Setup!");
			...
			setupData: {
					...
					let { directory } = await prompt.ask([
						{
							...
							name: "directory",
							message:
								"Where is your Foundry/Data directory located?",
				...
				filesystem
					.symlinkAsync(process.cwd(), dataPath)
					.catch((err) => {
			...
			while (!foundryBinary) {
				...
				foundryBinary = filesystem.resolve(
					path,
			...
			filesystem
				.writeAsync("foundryconfig.json", { data })
				.catch((err) => {
			...
			print.muted(
				"You can now run 'start' to build in development mode."
			);
			print.muted(
